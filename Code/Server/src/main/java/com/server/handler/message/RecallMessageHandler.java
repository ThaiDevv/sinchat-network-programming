...
public class RecallMessageHandler {
    private static final Logger logger = LoggerFactory.getLogger(RecallMessageHandler.class);
    ...
    public JsonObject handleTcp(JsonObject request, ClientConnection conn) {
        ...
        try {
            ...
            if (success) {
                ...
                // Broadcast recall event to all conversation members
                JsonObject event = new JsonObject();
                event.addProperty("action", "MESSAGE_RECALLED_EVENT");
                ...
                response.addProperty("status", "error");
                response.addProperty("message", "Failed to recall message");
            }
        } catch (Exception e) {
            logger.error("[RECALL_MESSAGE ERROR] Error: {}", e.getMessage(), e);
            response.addProperty("status", "error");
