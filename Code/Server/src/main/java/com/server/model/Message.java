...
public class Message {
    // Enum ánh xạ cột `type` ENUM('TEXT','IMAGE','VIDEO','VOICE','FILE','SYSTEM')
    public enum MessageType { TEXT, IMAGE, VIDEO, VOICE, FILE, SYSTEM }

    private long id;                    // bigint(20) NOT NULL AUTO_INCREMENT
    private long conversationId;        // bigint(20) NOT NULL — FK → conversations.id
    private long senderId;              // bigint(20) NOT NULL — FK → users.id
    private MessageType type;           // enum DEFAULT 'TEXT'
    private String content;             // text — nội dung tin nhắn
